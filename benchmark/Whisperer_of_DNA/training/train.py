...
import traceback
import h5py # <--- 添加 h5py 导入，用于保存权重示例
...
def train(args):
    ...
    if args.predict_only:
        ...
        # 获取 collate_fn（若有）
        if hasattr(datamodule, 'collate_fn'):
            collate_fn = datamodule.collate_fn
            print("从 DataModule 获取 collate_fn（推理）。")
        else:
            collate_fn = None
            print("警告: DataModule 未提供 collate_fn，将使用 DataLoader 默认值（推理）。")
    ...
    if cv_params['use_cv']:
        ...
        for fold_idx in range(cv_params['start_fold'], cv_params['n_splits']):
            ...
            if hasattr(datamodule, 'collate_fn'):
                ...
                collate_fn = None # 或者 torch.utils.data.default_collate
            ...
            if args.checkpoint:
                ...
                try:
                    ...
                    if 'state_dict' in checkpoint_data:
                        ...
                        if missing_keys:
                            print(f"警告 (折 {fold_idx + 1}): 加载权重时，模型中缺少以下键: {missing_keys}")
                        if unexpected_keys:
                            print(f"警告 (折 {fold_idx + 1}): 加载权重时，检查点中存在模型不需要的额外键: {unexpected_keys}")
                        print(f"预训练权重加载成功 (折 {fold_idx + 1})。将使用新配置继续训练。")
                    else:
                ...
                if best_ckpt_path:
                    ...
                    try:
                        ...
                        if train_dataset is None or val_dataset is None or test_dataset is None:
                             print("警告: 未能从 datamodule 获取所有数据集 (train/val/test)。可能需要调整 datamodule 实现以保留它们。")
                        print("数据集准备就绪。")

                        output_dir = fold_log_dir
                        print(f"预测结果将保存至: {output_dir}")

                        # --- 修改：调用 save_predictions_and_weights ---
                        print(f"\n开始为第 {fold_idx + 1} 折保存预测和池化权重 (Rank {trainer.global_rank})...")
                        for pred_stage_for_weights in ['train', 'val', 'test']:
                            ...
                            save_predictions_and_weights(
                                ...
                                output_dir=output_dir, # fold_log_dir
                                phenotype_names=phenotype_names_from_model,
                                data_loader_params=data_loader_params,
                                collate_fn=collate_fn,
                                trainer_config=predict_trainer_config
        ...
        if hasattr(datamodule, 'collate_fn'):
            collate_fn = datamodule.collate_fn
            print("从 DataModule 获取 collate_fn。")
        else:
            collate_fn = None # 或者 torch.utils.data.default_collate
            print("警告: DataModule 未提供 collate_fn，将使用 DataLoader 默认值。")
        # --- 结束修改 ---
        ...
        if args.checkpoint:
            ...
            try:
                ...
                if 'state_dict' in checkpoint_data:
                    ...
                    missing_keys, unexpected_keys = model.load_state_dict(model_state_dict, strict=False)
                    if missing_keys:
                        print(f"警告: 加载权重时，模型中缺少以下键: {missing_keys}")
                    if unexpected_keys:
                        print(f"警告: 加载权重时，检查点中存在模型不需要的额外键: {unexpected_keys}")
                    print("预训练权重加载成功。将使用新配置继续训练。")
                else:
                    print(f"警告: 检查点 {args.checkpoint} 中未找到 'state_dict'。将从头开始训练。")
            except FileNotFoundError:
                print(f"错误: 检查点文件未找到: {args.checkpoint}。将从头开始训练。")
            except Exception as e:
        ...
        if trainer.is_global_zero:
            if best_ckpt_path:
                ...
                try:
                    ...
                    # --- 修改：调用 save_predictions_and_weights ---
                    print(f"\n开始保存预测和池化权重 (Rank {trainer.global_rank})...")
                    for pred_stage_for_weights in ['train', 'val', 'test']:
                        print(f"  正在处理阶段: {pred_stage_for_weights}")
                        save_predictions_and_weights(
                            model=model_to_predict,
                            datamodule=datamodule,
                            stage=pred_stage_for_weights,
                            output_dir=output_dir, # exp_log_dir
                            phenotype_names=phenotype_names_from_model,
                            data_loader_params=data_loader_params,
                            collate_fn=collate_fn,
                            trainer_config=predict_trainer_config
