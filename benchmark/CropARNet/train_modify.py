    ...
    try:
        ...
        for param, (expected_type, description) in required_params.items():
            if param not in config:
                raise ConfigError(
                    f"Missing required parameter: {param} ({description})")
            if not isinstance(config[param], expected_type):
                raise ConfigError(
                    f"Invalid type for {param}: expected {expected_type.__name__}, got {type(config[param]).__name__}")
            if expected_type == float and config[param] <= 0:
                raise ConfigError(
                    f"Invalid value for {param}: must be positive")
    ...
    def __init__(self, num_snps, config):
        ...
        try:
            ...
            if not isinstance(num_snps, int) or num_snps <= 0:
                raise ValueError(
                    f"num_snps must be positive integer, got {num_snps}")
        ...
        try:
            ...
            for i, h_size in enumerate(self.config['weights_units']):
                if not isinstance(h_size, int) or h_size <= 0:
                    raise ValueError(
                        f"Invalid hidden size {h_size} in attention layer {i}")
    ...
    def _build_regressor_module(self, num_snps):
        ...
        try:
            ...
            for i, h_size in enumerate(self.config['regressor_units']):
                if not isinstance(h_size, int) or h_size <= 0:
                    raise ValueError(
                        f"Invalid hidden size {h_size} in regressor layer {i}")
                layers.append(nn.Linear(prev_size, h_size))
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, r2=None, pcc=None, path='./', col=None):
        ...
        try:
            ...
            if self.best_score is None:
                ...
                self.best_r2 = r2
                self.best_pcc = pcc
                self.save_checkpoint(val_loss, model, path, col)
    ...
    def save_checkpoint(self, val_loss, model, path, col):
        ...
        try:
            ...
            if not os.path.exists(save_path):
                raise RuntimeError(
                    f"Failed to verify saved model at {save_path}")
        except Exception as e:
...
def load_data(snp_path, pheno_path, col, config):
    ...
    try:
        ...
        try:
            ...
            snp_df = snp_data.iloc[1:, 1:].apply(
        ...
        try:
            ...
            if col >= pheno_data.shape[1]:
                raise IndexError(
                    f"Column index {col} out of bounds for phenotype data")

            phenotype_series = pheno_data.iloc[1:, col].copy()
            phenotype_df = phenotype_series.apply(
                lambda x: pd.to_numeric(x, errors='coerce'))

            if phenotype_df.isna().all():
                raise DataLoadingError(
                    f"All values in phenotype column {col} are invalid")

            fill_value = phenotype_df.mean()
            phenotype_values = phenotype_df.fillna(
                fill_value).values.astype(np.float32)
        except Exception as e:
...
def train_model(snp_path, pheno_path, result_txt_path, result_pdf_path, col, config):
    ...
    try:
        ...
        try:
            ...
            phenotype_name = pheno_data.iloc[0,
                                             col] if col < pheno_data.shape[1] else f"col_{col}"
        except:
...
{'='*80}"""
        ...
        X_train, X_test, y_train, y_test = load_data(
            snp_path, pheno_path, col, config)

        # Initialize model
        model = SimpleSNPModel(
            num_snps=X_train.shape[1], config=config).to(device)
        ...
        scaler = GradScaler('cuda', enabled=config['precision'] == 'mixed')
        ...
        for epoch in range(config['epochs']):
            try:
                ...
                with autocast('cuda', enabled=config['precision'] == 'mixed'):
...
Test Loss: {test_loss.item():.4f} | Test R²: {test_r2[-1]:.4f} | Test PCC: {test_pcc[-1]:.4f}"""
        ...
        try:
            ...
            att_weights_path = result_txt_path.replace(
                ".txt", f"_att_weights_{phenotype_name}.csv")
            pd.DataFrame(test_att_weights.cpu().detach().numpy()
                         ).to_csv(att_weights_path, index=False)

            # Save final model
            final_model_path = result_txt_path.replace(
                ".txt", f"_final_model_col_{col}.pth")
