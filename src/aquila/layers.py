...
class Activation(nn.Module):
    """Activation layer module for use in nn.Sequential.

    Wraps the activate function as a PyTorch module.
    """
    ...
    def forward(self, x):
        return activate(x, self.activation)
...
class CombinedGenomicEmbedding(nn.Module):
    ...
    def __init__(self, embed_dim=128, dropout=0.1):
        ...
        raise NotImplementedError(
            "Combined genomic embedding not yet implemented")
...
class MultiQueryAttentionRoPE(nn.Module):
    ...
    - RoPE on queries and keys (optional, default enabled)
    - Soft-clipping of attention logits for stability
    """

    def __init__(self, d_model, num_query_heads=8, qk_head_dim=128,
                 v_head_dim=192, dropout=0.1, max_position=8192, use_rope=True):
        super().__init__()
        self.d_model = d_model
        ...
        self.q_proj = nn.Linear(
            d_model, num_query_heads * qk_head_dim, bias=False)
        ...
        if self.use_rope:
            self.rope = RotaryPositionEmbedding(
                qk_head_dim, max_seq_len=max_position)
        else:
            self.rope = None
    ...
    def forward(self, x, mask=None):
        ...
        q = q.reshape(batch_size, seq_len,
                      self.num_query_heads, self.qk_head_dim)
        # Apply LayerNorm to Q
        q = self.q_norm(q)
        # (batch, num_query_heads, seq_len, qk_head_dim)
        q = q.permute(0, 2, 1, 3)
        ...
        out = out.transpose(1, 2).reshape(
            batch_size, seq_len, self.num_query_heads * self.v_head_dim)
    ...
    def forward(self, x, mask=None):
        ...
        if self.pool_axis == 1:
            ...
            if self.pool_type == 'mean':
                ...
                attn_logits = self.attention_weights(
...
class LearnableQueryPooling(nn.Module):
    ...
    def __init__(self, d_model, num_queries=1, num_layers=2, dropout=0.1, pool_axis=1):
        ...
        if pool_axis == 1:
            ...
            if self.use_lstm:
                self.lstm = nn.LSTM(
                    d_model, d_model, num_layers, batch_first=True)
    ...
    def forward(self, x, mask=None):
        ...
        if self.pool_axis == 1:
            ...
            query = self.query.unsqueeze(0).expand(
                batch_size, -1, -1)  # (batch, num_queries, d_model)
            ...
            # (batch, seq_len, d_model)
            attn_logits = x * self.feature_weight.unsqueeze(0).unsqueeze(0)
...
class HierarchicalPooling(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        concat = torch.cat([local_agg, global_feat],
...
class TransformerPooling(nn.Module):
    ...
    def __init__(self, d_model, num_layers=2, num_heads=8, dim_feedforward=None, dropout=0.1, pool_axis=1):
        ...
        if pool_axis == 1:
            ...
            # Use smaller embedding for efficiency
            feature_embed_dim = min(64, d_model)
            if dim_feedforward is None:
            ...
            encoder_layer = nn.TransformerEncoderLayer(
                ...
                # Adjust heads based on embedding dim
                nhead=min(4, feature_embed_dim // 16),
                dim_feedforward=dim_feedforward,
                dropout=dropout,
                batch_first=True,
                activation='gelu'
    ...
    def forward(self, x, mask=None):
        ...
        if self.pool_axis == 1:
            ...
            pool_token = self.pool_token.expand(
                batch_size, -1, -1)  # (batch, 1, d_model)
            # (batch, seq_len+1, d_model)
            x_with_pool = torch.cat([pool_token, x], dim=1)

            # Extend mask to include pooling token (always valid)
            if mask is not None:
                pool_mask = torch.ones(
                    batch_size, 1, device=mask.device, dtype=torch.bool)
            ...
            encoded = self.transformer(
                x_with_pool, src_key_padding_mask=key_padding_mask)
            ...
            # (batch * seq_len, d_model, feature_embed_dim)
            x_embedded = self.feature_projection(x_reshaped)

            # Add CLS token for each position
            pool_token = self.pool_token.expand(
                batch_size * seq_len, -1, -1)  # (batch * seq_len, 1, 1)
            pool_token_embedded = self.feature_projection(
                pool_token)  # (batch * seq_len, 1, feature_embed_dim)

            # Concatenate CLS token with feature embeddings
            # (batch * seq_len, d_model+1, feature_embed_dim)
            x_with_pool = torch.cat([pool_token_embedded, x_embedded], dim=1)

            # Apply transformer to aggregate features
            # (batch * seq_len, d_model+1, feature_embed_dim)
            encoded = self.transformer(x_with_pool)

            # Extract CLS token representation
            # (batch * seq_len, feature_embed_dim)
            cls_output = encoded[:, 0, :]

            # Project back to scalar
            pooled = self.output_projection(
                cls_output).squeeze(-1)  # (batch * seq_len,)
...
class SampleStructureEncoder(nn.Module):
    ...
    def __init__(self, d_model, structure_dim=64, use_cross_sample_attention=True,
        ...
        if pooling_type == 'attention':
            ...
            self.sample_pooling = GlobalPooling(
                d_model, pool_type='attention', pool_axis=pool_axis)
        elif pooling_type == 'multi_head':
            # Use existing MultiHeadPooling (combines mean, max, attention, std)
            self.sample_pooling = MultiHeadPooling(
                d_model, num_heads=4, dropout=dropout, pool_axis=pool_axis)
            ...
            self.sample_pooling = GlobalPooling(
                d_model, pool_type='mean', pool_axis=pool_axis)
    ...
    def forward(self, x, mask=None):
        ...
        if self.sample_pooling is not None:
            ...
            # (batch, d_model) or (batch, seq_len)
            sample_repr = self.sample_pooling(x, mask=mask)
        elif self.pooling_type == 'mean_max':
            # Combine mean and max pooling
            if mask is not None:
                x_masked = x * mask.unsqueeze(-1)
                mean_pooled = x_masked.sum(
                    dim=1) / mask.sum(dim=1, keepdim=True).clamp(min=1)
            ...
            sample_repr = (mean_pooled + max_pooled) / \
                2  # Average of mean and max
        else:
            # Fallback to mean pooling
            if mask is not None:
                x_masked = x * mask.unsqueeze(-1)
                sample_repr = x_masked.sum(
                    dim=1) / mask.sum(dim=1, keepdim=True).clamp(min=1)
            else:
        ...
        structure_features = self.stat_extractor(
            sample_repr)  # (batch, structure_dim)

        # Project to model dimension
        structure_emb = self.structure_projection(
            structure_features)  # (batch, d_model)

        # Option 1: Additive injection (like positional encoding)
        # (batch, seq_len, d_model)
        x_enhanced = x + structure_emb.unsqueeze(1)
        ...
        if self.use_cross_sample_attention:
            ...
            structure_emb_expanded = structure_emb.unsqueeze(
                1)  # (batch, 1, d_model)
...
class SampleCLSToken(nn.Module):
    ...
    def __init__(self, d_model, num_layers=2, num_heads=8, dim_feedforward=None,
        ...
        self.sample_transformer = nn.TransformerEncoder(
            encoder_layer, num_layers=num_layers)
    ...
    def forward(self, x, mask=None):
        ...
        if self.sample_pooling is not None:
            # Use existing pooling module (GlobalPooling or MultiHeadPooling)
            # (batch, d_model) or (batch, seq_len)
            sample_summary = self.sample_pooling(x, mask=mask)
        elif self.pooling_type == 'mean_max':
            # Combine mean and max pooling
            if mask is not None:
                x_masked = x * mask.unsqueeze(-1)
        ...
        # (batch, seq_len+1, d_model)
        x_with_cls = torch.cat([sample_cls_expanded, x], dim=1)
        ...
        encoded = self.sample_transformer(
            x_with_cls, src_key_padding_mask=key_padding_mask)
        ...
        # (batch, seq_len, d_model)
        x_enhanced = enhanced_seq + cls_injected.unsqueeze(1)
...
class RotaryPositionEmbedding(nn.Module):
    ...
    def forward(self, q, k):
        ...
        # Apply rotation
        q_rot = (q * self._cos_cached) + \
            (self._rotate_half(q) * self._sin_cached)
        k_rot = (k * self._cos_cached) + \
            (self._rotate_half(k) * self._sin_cached)
