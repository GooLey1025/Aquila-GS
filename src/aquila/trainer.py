...
class SNPTrainer:
    ...
    def __init__(
        ...
        if self.scheduler_type == 'reduce_on_plateau':
            ...
            if num_epochs is None:
                raise ValueError("num_epochs must be provided for OneCycleLR scheduler")
            ...
            if num_epochs is None:
                raise ValueError("num_epochs must be provided for cosine_warmup scheduler")
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch in self.train_loader:
            ...
            if 'regression_targets' in batch:
                targets['regression'] = batch['regression_targets'].to(self.device)
            ...
            if 'classification_targets' in batch:
                targets['classification'] = batch['classification_targets'].to(self.device)
                masks['classification'] = batch['classification_mask'].to(self.device)
            ...
            if 'regression' in predictions:
                all_predictions.append(predictions['regression'].detach().cpu().numpy())
            ...
            # Step per-batch schedulers after each batch
    ...
    def validate(self) -> Tuple[Dict[str, float], Dict[str, np.ndarray]]:
        ...
        for batch in self.val_loader:
            ...
            if 'regression_targets' in batch:
                targets['regression'] = batch['regression_targets'].to(self.device)
                masks['regression'] = batch['regression_mask'].to(self.device)
                all_targets['regression'].append(targets['regression'].cpu().numpy())
                all_masks['regression'].append(masks['regression'].cpu().numpy())
            
            if 'classification_targets' in batch:
                targets['classification'] = batch['classification_targets'].to(self.device)
                masks['classification'] = batch['classification_mask'].to(self.device)
                all_targets['classification'].append(targets['classification'].cpu().numpy())
                all_masks['classification'].append(masks['classification'].cpu().numpy())
            ...
            # Store predictions
            if 'regression' in predictions:
                all_predictions['regression'].append(predictions['regression'].cpu().numpy())
            if 'classification' in predictions:
                all_predictions['classification'].append(predictions['classification'].cpu().numpy())
    ...
    def train(self, num_epochs: int, verbose: bool = True) -> Dict[str, list]:
        ...
        for epoch in range(num_epochs):
            ...
            self.history['train_r'].append(train_metrics.get('train_pearson', 0.0))
            ...
            if self.val_loader is not None:
                ...
                self.history['val_r'].append(val_metrics.get('val_pearson', 0.0))
