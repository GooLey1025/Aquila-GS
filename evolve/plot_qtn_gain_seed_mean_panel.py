...
def parse_args():
    ...
    parser.add_argument(
        "--screening-name",
        default="screening_si_per_round.tsv",
        help="Screening file name inside each seed directory."
    )
    ...
    parser.add_argument(
        "--gain-col",
        default="SI_gain",
        help="Column name for per-SNP SI gain. Use 'SI_gain' for true per-SNP contribution, or 'gain' for cumulative SI."
    )
...
def load_evolved_major_gt_across_seed_vcfs(seed_dirs, evolved_vcf_name, site_ids):
    ...
    for seed_dir in seed_dirs:
        ...
        gt_dict = load_one_seed_evolved_gt(vcf, site_set)
...
def draw_gene_snp_label(ax, x, y, gene, snp_id, fontsize=8.6):
    gene_area = TextArea(
        str(gene),
        textprops=dict(
            fontsize=fontsize,
...
def draw_rect_label(ax, x, y, text, color, width=None, height=0.50, fontsize=8.2):
    if width is None:
        width = 0.85 + len(text) * 0.055
...
def main():
    ...
    teqing_gt = load_single_sample_gt(args.teqing_vcf, site_ids, "Teqing")
    hhz_gt = load_single_sample_gt(args.hhz_vcf, site_ids, "Huanghuazhan")
    ...
    ax_pattern.text(0.5, n + 0.35, "Evolutionary\npattern", ha="center", va="bottom", fontsize=11, fontweight="bold")

    ax_gt.text(1.5, n + 0.7, "Genotype", ha="center", va="bottom", fontsize=11, fontweight="bold")
    ...
    for i, (_, r) in enumerate(matched.iterrows()):
        ...
        draw_gt_box(ax_gt, 0, i, gt_display(r["Teqing_GT"]), genotype_colors["Teqing"])
        draw_gt_box(ax_gt, 1.5, i, gt_display(r["HHZ_GT"]), genotype_colors["HHZ"])
        draw_gt_box(ax_gt, 3.0, i, gt_display(r["Evolved_Major_GT"]), genotype_colors["Evolved"])
